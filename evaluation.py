...
def call_llm(user_stories, prompt):
    from UtilityFunctions import get_completion_from_messages
    messages = [{"role": "user", "content": prompt}]
    response = get_completion_from_messages(user_stories,prompt)
    return response
...
def single_prompt(user_stories):
    prompt = f"""
You are an expert in software engineering and domain modeling.
...
Guidelines:
- Class names must follow camelCase naming convention.
- Relationships must be meaningful verbs connecting two classes.
- Avoid duplicate or redundant classes.
...
""".replace("<USER_STORIES>", user_stories)

    response = call_llm(user_stories, prompt)
    return extract_json(response)


# ---------------------------
# 🧪 BASELINE 2: MULTI-STEP (NO DECOMPOSITION)
# ---------------------------

def multi_step(user_stories):
...
""".replace("<USER_STORIES>", user_stories)
    ...
    res1 = extract_json(call_llm(user_stories,prompt1))
    class_list = [c["name"] for c in res1.get("classes", [])]
...
Identify relationships between these classes.

Guidelines:
- Relationships must be verbs
- Only use classes from the given list
...
Output only JSON.
"""
    prompt2 = prompt2.replace("<USER_STORIES>", user_stories)
    prompt2 = prompt2.replace("<CLASS_LIST>", str(class_list))

    res2 = extract_json(call_llm(user_stories,prompt2))

    return {
        "classes": res1.get("classes", []),
        "relationships": res2.get("relationships", [])
    }


# ---------------------------
# 🧪 BASELINE 3: NO FEW-SHOT (RELATIONSHIPS)
# ---------------------------

def no_few_shot(user_stories):

    # Step 1: Classes (same as multi-step)
    prompt1 = f"""
You are an expert in software engineering.
...
""".replace("<USER_STORIES>", user_stories)
    ...
    # Step 2: Relationships WITHOUT few-shot examples
    prompt2 = f"""
Find relationships between the following classes: <CLASS_LIST> involved in the user stories.

Go through each individual user story first, identify relationships, then aggregate.

NOTE:
In the relationship 'KitchenEmployee receives Product':
KitchenEmployee is class,
receives is relationship name,
Product is class.
...
"""
    ...
    print("\n--- SINGLE PROMPT ---")
    print(json.dumps(single_prompt(sample), indent=2))

    print("\n--- MULTI STEP ---")
    print(json.dumps(multi_step(sample), indent=2))

    print("\n--- NO FEW SHOT ---")
    print(json.dumps(no_few_shot(sample), indent=2))